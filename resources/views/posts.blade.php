  ...
  <div class="col-md-6">
    <form action="/blog">
      ...
      <div class="input-group mb-3">
        ...
        <button class="btn btn-dark btn-outline-primary text-light" type="submit">Search</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="card-body text-center">
    ...
    <p>
      <small class="text-muted">
        By. <a href="/blog?author={{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name}}</a> in <a href="
        /blog?category={{ $posts[0]->category->slug }}"class="text-decoration-none">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}
      </small>
   </p>
